...
export const zh: I18nDictionary = {
  ...
  "cmd.description.mcp": "安装新的 MCP 服务器",
  ...
  "cmd.description.soul": "查看助手个性 (soul.md)",
  ...
  "mcp.install.prompt.name":
    "🔌 发送 MCP 服务器的简短名称（字母、数字、_ 和 -）。示例：filesystem",
  "mcp.install.prompt.type": "选择 MCP 服务器类型：",
  "mcp.install.prompt.command":
    "📝 发送启动 MCP 服务器的完整命令。\n\n示例：\nnpx -y @modelcontextprotocol/server-filesystem /workspace",
  "mcp.install.prompt.url":
    "🌐 发送远程 MCP 服务器的 URL。\n\n示例：\nhttps://mcp.example.com/v1/sse",
  "mcp.install.prompt.scope": "选择安装范围：",
  "mcp.install.prompt.env":
    "🔐 环境变量（可选）。\n\n发送 KEY=值（每行一对），上传文件（.env 或 Google OAuth credentials.json），或发送 /skip 跳过。",
  "mcp.install.env_invalid":
    "⚠️ 格式无效。每行须为 KEY=值（KEY 仅限 A-Z、0-9 和 _）。",
  "mcp.install.env_skipped": "✅ 无环境变量。",
  "mcp.install.json_parsed_oauth": "✅ 已识别 Google OAuth 凭据。已提取：{keys}",
  "mcp.install.json_parsed_env": "✅ 从文件加载了 {count} 个环境变量。",
  "mcp.install.json_unrecognized":
    "⚠️ 文件不是已识别的 credentials.json 或 .env。请手动发送 KEY=值 或 /skip。",
  "mcp.install.file_too_large": "⚠️ 文件过大。最大 64KB。",
  "mcp.install.file_download_error": "🔴 无法下载上传的文件。",
  "mcp.install.preview":
    "安装前请检查 MCP 服务器：\n\n名称：{name}\n类型：{type}\n范围：{scope}\n{spec}{envBlock}",
  "mcp.install.preview.command": "命令：{command}",
  "mcp.install.preview.url": "URL：{url}",
  "mcp.install.preview.env_keys": "\n环境变量（{count}）：{keys}",
  "mcp.install.tunnel_disclaimer":
    "ℹ️ 该 MCP 看起来基于 OAuth。OAuth 首次使用时会打开回调 URL。\n\n• 本机 PC：开箱即用。\n• Docker / VPS / Droplet：localhost 回调无法到达，需要公网 HTTPS 隧道指向 OpenCode 容器，然后设置 GOOGLE_OAUTH_REDIRECT_URI / WORKSPACE_EXTERNAL_URL。\n\n免费且无需审核的选项：\n• ngrok — `ngrok http 8000`\n• Cloudflared（TryCloudflare）— `cloudflared tunnel --url http://localhost:8000`\n\n在 Google Cloud Console 中将得到的公网 URL 添加为 OAuth 客户端的 redirect URI。",
  "mcp.install.scope.global": "🌐 全局（所有会话）",
  "mcp.install.scope.project": "📁 仅当前项目",
  "mcp.install.scope.global_label": "全局",
  "mcp.install.scope.project_label": "项目",
  "mcp.install.button.local": "🖥️ 本地",
  "mcp.install.button.remote": "🌐 远程",
  "mcp.install.button.global": "🌐 全局",
  "mcp.install.button.project": "📁 仅当前项目",
  "mcp.install.button.confirm": "✅ 安装",
  "mcp.install.button.cancel": "❌ 取消",
  "mcp.install.name_invalid": "⚠️ 名称无效。仅限字母、数字、_ 和 -，最多 64 字符。",
  "mcp.install.name_taken": "⚠️ 已存在同名 MCP 服务器。",
  "mcp.install.command_empty": "⚠️ 命令不能为空。",
  "mcp.install.url_invalid": "⚠️ URL 无效。请发送有效的 http(s):// URL。",
  "mcp.install.cancelled": "❌ MCP 安装已取消。",
  "mcp.install.cancel_callback": "正在取消...",
  "mcp.install.installing": "⏳ 正在安装 MCP 服务器...",
  "mcp.install.success": "✅ MCP 已安装：{name}（{type}，{scope}）。状态：{status}",
  "mcp.install.connect_failed": "⚠️ MCP 已安装但连接失败：{error}",
  "mcp.install.error": "🔴 MCP 服务器安装失败：{error}",
  "mcp.install.inactive_callback": "此 MCP 安装流程已不再活动",
  "mcp.install.inactive": "⚠️ MCP 安装未激活。请再次运行 /mcp。",
  "mcp.install.project_not_selected":
    "🏗 未选择项目。请使用 /projects 或选择全局范围。",
